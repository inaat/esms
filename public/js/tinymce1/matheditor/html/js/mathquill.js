...
(function () {
    ...
    function createRoot(jQ, root, textbox, editable) {
        ...
        jQ.bind('mousedown.mathquill', function (e) {
            // For mobile - for some reason it does not bring up the keyboard for a mousedown event so we
            // simply force the keyboard by focusing the text area
            textarea.focus();
    ...
    LatexCmds.matrix = P(MathCommand, function (_, _super) {
        _.ctrlSeq = '\\matrix';
        _.htmlTemplate =
          '<span class="mtable non-leaf">'
          + '<span class="mtr non-leaf">'
          + '<span class="mtd"><span>a</span></span>'
          + '<span class="mtd"><span>b</span></span>'
          + '</span>'
          + '<span class="mtr non-leaf">'
          + '<span class="mtd"><span>c</span></span>'
          + '<span class="mtd"><span>d</span></span>'
          + '</span>'
          + '</span>'
        ;
        _.latex = function () {
        ...
        _.setSize = MatrixSize = function (rows, columns) {
            _.columns = columns;
            var matrix = '<span class="mtable non-leaf">';
            var count = 0;
            for (var row = 0; row < rows; row++) {
                matrix += '<span class="mtr non-leaf">';
                for (var col = 0; col < columns; col++) {
                    matrix += '<span class="mtd"><span>&' + count + '</span></span>';
                    count++;
                }
                matrix += '</span>';
            }
    ...
    LatexCmds.notequiv = bind(BinaryOperator, '\\not\\equiv ', '&#8802;');
    ...
    LatexCmds.notsim = bind(BinaryOperator, '\\not\\sim ', '&#8769;');
    LatexCmds.neq = bind(BinaryOperator, '\\neq ', '&#8800;');
    LatexCmds.lneq = bind(BinaryOperator, '\\lneq ', '&#10887;');
    LatexCmds.gneq = bind(BinaryOperator, '\\gneq ', '&#10888;');
    ...
    */
    ...
    LatexCmds.leftmapsto = bind(VanillaSymbol, '\\leftmapsto ', '&#8612;');
    ...
    LatexCmds.rightbarharpoon = bind(VanillaSymbol, '\\rightbarharpoon ', '&#10605;');
    LatexCmds.leftbarharpoon = bind(VanillaSymbol, '\\leftbarharpoon ', '&#10602;');
    LatexCmds.swarrow = bind(VanillaSymbol, '\\swarrow ', '&#8601;');
    LatexCmds.leftharpoondown = bind(VanillaSymbol, '\\leftharpoondown ', '&#8637;');
    LatexCmds.rightharpoondown = bind(VanillaSymbol, '\\rightharpoondown ', '&#8641;');
    LatexCmds.nwarrow = bind(VanillaSymbol, '\\nwarrow ', '&#8598;');
    LatexCmds.dlsh = bind(VanillaSymbol, '\\dlsh ', '&#8629;');
    ...
    //LatexCmds.ddots = 
    LatexCmds.upslopeellipsis = bind(VanillaSymbol, '\\upslopeellipsis ', '&#8944;');
    ...
    //variable-sized
    LatexCmds.surfintegral = bind(VanillaSymbol, '\\surfintegral', '&#8751;');
    LatexCmds.oint = bind(VanillaSymbol, '\\oint ', '&#8750;');
    LatexCmds.iint = bind(VanillaSymbol, '\\iint ', '&#8748;');
    LatexCmds.iiint = bind(VanillaSymbol, '\\iiint ', '&#8749;');
    LatexCmds.volintegral = bind(VanillaSymbol, '\\volintegral ', '&#8752;');
    ...
    var latexMathParser = (function () {
        ...
        var matrixCommand =
          regex(/^\\begin{array}\{(.*?)\}/)
          .then(regex(/^(.*?)\\end{array}/))
          .then(function (a) {
              // Strip out the trailing command (\end{matrix})
              var content = a.replace(/\\end{array}/, '');
