...
class ModuleUtil extends Util
{
    ...
    {
        return [
            'purchases' => ['name' => __('english.purchases')],
            'add_sale' => ['name' => __('sale.add_sale')],
            'pos_sale' => ['name' => __('sale.pos_sale')],
            'stock_transfers' => ['name' => __('english.stock_transfers')],
            'stock_adjustment' => ['name' => __('stock_adjustment.stock_adjustment')],
            'expenses' => ['name' => __('english.expenses')],
            'account' => ['name' => __('english.account')],
            ...
            'booking' => ['name' => __('english.enable_booking')],
            'kitchen' => [
                'name' => __('restaurant.kitchen_for_restaurant')
            ],
            'subscription' => ['name' => __('english.enable_subscription')],
            'types_of_service' => ['name' => __('english.types_of_service'),
                        'tooltip' => __('english.types_of_service_help_long')
                    ]
        ];
    }
    ...
    public function getTaxonomyData($category_type)
    {
        ...
        foreach ($modules_data as $module => $data) {
            foreach ($data  as $key => $value) {
                ...
                if (!in_array($key, $category_types)) {
                    ...
                    echo __('english.duplicate_taxonomy_type_found');
                    exit;
                }
        ...
        if (!in_array($category_type, $category_types)) {
            echo __('english.taxonomy_type_not_found');
            exit;
        }
        return $module_data;
    }
