...
class StudentUtil extends Util
{
    ...
    public function updateStudent($request, $student_id, $guardian_link_id = null)
    {
        ...
        if (!empty($ob_transaction)) {
            $opening_balance_paid = $this->feeTransactionUtil->getTotalAmountPaid($ob_transaction->id);
            if (!empty($opening_balance<$opening_balance_paid)) {
                $output = ['success' => false,
                    'msg' => __("english.student_details_is_update_but_the_opening_balance_amount_less_than_paid_amount")
                ];
                return $output;
            }
