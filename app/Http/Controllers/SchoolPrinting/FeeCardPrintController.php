    ...
    public function singlePrint($id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                // $transaction=FeeTransaction::with(['session','campus','student','fee_lines.feeHead','student_class','student_class_section'])->find($id);
                // $date = \Carbon::parse($transaction->transaction_date);
                // $year= $date->year;
                // $other_transaction=FeeTransaction::with(['payment_lines','session','campus','student','fee_lines.feeHead','student_class','student_class_section'])
                //     ->whereIn('type',['other_fee','admission_fee'])
                //     ->where('month', $transaction->month)
                //     ->where('student_id', $transaction->student_id)
                //     ->whereYear('transaction_date', $year)->get();
                // $transport_transaction=FeeTransaction::with(['payment_lines','session','campus','student','fee_lines.feeHead','student_class','student_class_section'])
                //     ->whereIn('type',['transport_fee'])
                //     ->where('month', $transaction->month)
                //     ->where('student_id', $transaction->student_id)
                //     ->whereYear('transaction_date', $year)->get();
                // $current_transaction=FeeTransaction::with(['payment_lines','session','campus','student','fee_lines.feeHead','student_class','student_class_section'])
                //     ->where('type','fee')
                //     ->where('month', $transaction->month)
                //     ->where('student_id', $transaction->student_id)
                //     ->whereYear('transaction_date', $year)->first();

                // $student_dues=$this->studentUtil->getStudentDue($transaction->student_id);
                // //dd($current_transaction);
                // $feeCards[]=[
                //     'current_transaction'=>$current_transaction,
                //     'other_transaction'=>$other_transaction,
                //     'transport_transaction'=>$transport_transaction,
                //     'total_fee_due'=>$student_dues->total_fee-$student_dues->total_fee_paid,
                //     'total_other_fee_due'=>$student_dues->total_other_fee-$student_dues->total_other_fee_paid+($student_dues->total_admission_fee-$student_dues->total_admission_fee_paid),
                //     'total_transport_fee_due'=>$student_dues->total_transport_fee-$student_dues->total_transport_fee_paid


                // ];
                // $student=$feeCards;
                // $receipt = $this->receiptContentFee($student);
        ...
        //        dd($discount_payment_formatted);
                ...
                $latest_transaction=FeeTransaction::where('student_id',$current_transaction->student_id)->latest('month')->first();
    ...
    public function createClassWisePrint(Request $request)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                if (empty($campus_id) || empty(request()->input('month_year'))|| empty($class_id) || empty($section_id)) {
                    $output = ['success' => 0,
